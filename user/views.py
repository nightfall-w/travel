...
class UserLogin(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if re.match(phone_pat, username):
            try:
                username = auth_profile.objects.get(phone_number=username).user_obj.username
        ...
        if not user:
            ...
            login(request, user)
            return Response({'success': True, 'message': '登录成功', 'username': username})
...
class UserRegister(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if register_username and not phone_number:
            if User.objects.filter(username=register_username):
                return Response({'return_code': False, 'return_value': '用户名已经存在'})
            else:
        ...
        if user_phone:
            return Response({'return_code': False, 'return_value': '该手机号已经被注册'})
        else:
            verification_code = create_verification()  # 产生一个6位的随机验证码
            request.session[phone_number] = verification_code
            request.session.set_expiry(300)
            request_to_chit_platform.delay(phone_number, verification_code)  # 向第三方平台发送该手机号验证码的请求
    ...
    def post(self, request, *args, **kwargs):
        ...
        logger.debug("session_verification_code:{}".format(session_verification_code))

        # 检查表单数据
        f = userRegister(request.POST)
        if not f.is_valid():
            return Response({'return_code': False, 'return_type': 'param_error', 'return_value': f.errors})
        if input_verification_code != session_verification_code:
            return Response({'return_code': False, 'return_type': 'verification_code_error', 'return_value': '验证码不正确'})
        else:
            try:
                user = User.objects.create_user(username=username)
            except:
                return Response({'return_code': False, 'return_type': 'username_exist', 'return_value': '用户名已经存在'})
            user.set_password(password)
            user.save()
            auth_profile.objects.create(phone_number=phone_number, user_obj=user)
